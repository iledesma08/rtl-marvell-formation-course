...
- **`φ` se expresa en "unidades de π"** (valor = ángulo/π). El formato `S(16,14)` cubre `[-2, 2)`, que **no alcanza para ±π** (~3.14). Normalizando a π, el círculo completo `(-π, π]` mapea a `(-1, 1]`, que entra holgado. La corrección de cuadrante pasa a ser sumar/restar `1.0` exacto (= π), y la tabla de `atan(2⁻ⁱ)` queda en esas mismas unidades.
...
- **Pre-escalado por `K = 0.60725`**: lo hacemos como **multiplicación por constante** (`X·K`), que el sintetizador convierte en sumas y desplazamientos — el CORDIC no usa multiplicador real. Cuidado con el ancho: el producto `x·K` necesita 32 bits (lo vimos desbordar en 16).
...
### Área (Yosys: `flatten + techmap + stat`)
...
> El CORDIC no tiene memoria: su tablita de 16 valores queda absorbida en la lógica combinacional. La referencia necesita 2 Mbit (2 × 64K × 16), que en un FPGA Artix-7 son ~64 BRAMs de 32 Kb. Si sumamos lógica + memoria, la referencia domina claramente el área. La lógica del CORDIC es algo mayor porque lleva *barrel shifters* (desplazamientos por `i` variable) y el escalado por `K`; la referencia, en cambio, tiene el multiplicador y el divisor.
...
- **Yosys** verifica el área de ambas arquitecturas (`run.sh` paso 4).
- **`analisis_error.py`** (numpy + PIL, sin matplotlib) genera el gráfico y el resumen por cuadrante.
...
Genera las ROMs, compila, simula (PASS/FAIL + tabla de error por cuadrante), compara el área con Yosys y dibuja `output_ej3.png`. Los datos generados (ROMs, vectores, `errores.csv`, el binario `sim.out` y el VCD) quedan en `datos/`. Para ver las ondas:

```bash
gtkwave datos/tb_cordic_vect.vcd
```